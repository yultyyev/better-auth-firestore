...
- 1.7 identifies accounts by `(issuer, accountId)`; `account.issuer` is required. Firestore has no `auth migrate`, so `backfillAccountIssuers` (`src/backfill-account-issuers.ts`, exposed as the CLI above) stamps existing documents. Rules mirror `@better-auth/core`: `credential` → `local:credential` (with `accountId = userId`), `siwe` → `local:siwe`, `google`/`apple`/`facebook`/`line` → the fixed real issuer their provider factory declares, and every provider that declares none → `local:oauth:<encodeURIComponent(providerId)>`. Providers whose real issuer depends on configuration or on the token (`cognito`, `paybin`, `microsoft`, and the fixed-id generic-OAuth helpers `okta`, `auth0`, `keycloak`, `microsoft-entra-id`) are **left unresolved** rather than guessed — supply them via `issuers` / `resolveIssuer` / `--issuer`. Do not "simplify" this back to a blanket `local:oauth:<providerId>`: that was the v1.3.0 bug, and it silently locked out every Google user until sign-in failed in production. Documents that v1.3.0 already mis-stamped are detected and repaired (`legacyIssuersRepaired`) even without `overwrite`, since 1.7 itself never writes that pairing. `warnIfAccountsLackIssuer` in `src/firebase-adapter.ts` runs once per process when the adapter is bound to a `betterAuth()` instance whose schema has `issuer` (two `count()` aggregations; `orderBy(issuer)` excludes documents missing the field) and warns with the exact command.
