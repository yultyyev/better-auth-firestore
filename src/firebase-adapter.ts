 ...
 */
function resolveCollectionNames(namingStrategy?: NamingStrategy, overrides?: CollectionsOverride) {
	...
	return {
		...
		verificationTokens: overrides?.verificationTokens ?? (snake ? `verification_tokens${suffix}` : `verificationTokens${suffix}`),
	};
}
...
function getCollectionRef(
	...
	if (normalized === "verificationtoken") return db.collection(collections.verificationTokens);
...
function applyWhereClause(
	...
	const firestoreConditions = andConditions.filter((w) => {
		...
		return op !== "notIn" && op !== "not_in" && op !== "endsWith" && op !== "ends-with" && op !== "ends_with" && op !== "contains";
	});
	let q = query;
...
export const firestoreAdapter = (config: FirestoreAdapterOptions | Firestore = {}) => {
	const db = resolveDb(config as any);
	const {
		namingStrategy = "default",
		collections: collectionsOverride = {},
		debugLogs = false,
	} = (config as FirestoreAdapterOptions && (config as any).collection ? {} : (config as FirestoreAdapterOptions)) || {};

	const preferSnakeCase = namingStrategy === "snake_case";
	const collections = resolveCollectionNames(namingStrategy, collectionsOverride);
		...
		adapter: () => {
			return {
				create: async ({ model, data }) => {
					...
					if (debugLogs) {
						console.log(`[Firestore Adapter] CREATE ${model}:`, { input: data, docData, collection: collections, collectionRef: col.path, docId: ref.id });
					}
					await ref.set(docData);
					const created = await ref.get();
					if (debugLogs) {
						console.log(`[Firestore Adapter] CREATE ${model} - document exists after set:`, created.exists, "path:", created.ref.path);
					}
					// Double-check by reading from the same ref
					if (debugLogs && created.exists) {
						const verifyDoc = await ref.get();
						console.log(`[Firestore Adapter] CREATE ${model} - verification read:`, verifyDoc.exists, "path:", verifyDoc.ref.path);
					}
					const result: any = { id: created.id };
					const createdData = created.data();
					if (debugLogs) {
						console.log(`[Firestore Adapter] CREATE ${model} - stored data:`, createdData);
					}
					if (createdData) {
					...
					if (debugLogs) {
						console.log(`[Firestore Adapter] CREATE ${model} - returning:`, { ...data, ...result });
					}
					return { ...data, ...result };
				},
				update: async ({ model, where, update }) => {
					...
					if (where && where.length === 1 && where[0]?.field === "id" && (where[0]?.operator === "eq" || !where[0]?.operator)) {
						const docId = where[0].value as string;
						const docRef = col.doc(docId);
						if (debugLogs) {
							console.log(`[Firestore Adapter] UPDATE ${model}:`, { where, update, "using direct doc lookup": docId });
						}
						const doc = await docRef.get();
						if (!doc || !doc.exists) {
							if (debugLogs) {
								console.log(`[Firestore Adapter] UPDATE ${model} - no document found by ID`);
							}
							return null as any;
						}
						...
						if (debugLogs) {
							console.log(`[Firestore Adapter] UPDATE ${model} - returning:`, result);
						}
						return result;
					}
					...
					const q = applyWhereClause(col, where, mapper);
					if (debugLogs) {
						console.log(`[Firestore Adapter] UPDATE ${model}:`, { where, update, collection: collections });
					}
				...
				delete: async ({ model, where }) => {
					...
					if (where && where.length === 1 && where[0]?.field === "id" && (where[0]?.operator === "eq" || !where[0]?.operator)) {
						...
						const docRef = col.doc(docId);
						if (debugLogs) {
							console.log(`[Firestore Adapter] DELETE ${model}:`, { where, "using direct doc lookup": docId });
						}
						const doc = await docRef.get();
						if (doc && doc.exists) {
							await docRef.delete();
			...
			findOne: async ({ model, where, select }) => {
				...
				if (where && where.length === 1 && where[0]?.field === "id" && (where[0]?.operator === "eq" || !where[0]?.operator)) {
					...
					const docRef = col.doc(docId);
					if (debugLogs) {
						console.log(`[Firestore Adapter] FINDONE ${model}:`, { where, select, collection: collections, collectionRef: col.path, "using direct doc lookup": docId, docPath: docRef.path });
					}
					const doc = await docRef.get();
					if (!doc || !doc.exists) {
						if (debugLogs) {
							console.log(`[Firestore Adapter] FINDONE ${model} - no document found by ID`, { docId, docPath: docRef.path, collectionPath: col.path });
						}
						return null as any;
					}
					const data = doc.data();
					if (debugLogs) {
						console.log(`[Firestore Adapter] FINDONE ${model} - raw Firestore data:`, data);
						console.log(`[Firestore Adapter] FINDONE ${model} - doc.id:`, doc.id);
						console.log(`[Firestore Adapter] FINDONE ${model} - data keys:`, Object.keys(data || {}));
					}
					...
					for (const [k, v] of Object.entries(data)) {
						...
						if (debugLogs) {
							console.log(`[Firestore Adapter] FINDONE ${model} - mapped field:`, { dbField: k, appField: fieldName, rawValue: v, convertedValue });
						}
					}

					if (debugLogs) {
						console.log(`[Firestore Adapter] FINDONE ${model} - result before select:`, result);
					}
					...
					if (select && select.length > 0) {
						...
						if (debugLogs) {
							console.log(`[Firestore Adapter] FINDONE ${model} - returning selected:`, selected);
						}
						return selected;
					}

					if (debugLogs) {
						console.log(`[Firestore Adapter] FINDONE ${model} - returning result:`, result);
					}
					return result;
				}
				...
				const q = applyWhereClause(col, where, mapper);
				if (debugLogs) {
					console.log(`[Firestore Adapter] FINDONE ${model}:`, { where, select, collection: collections });
				}
				const snap = await q.limit(1).get();
				if (debugLogs) {
				...
				findMany: async ({ model, where, limit, offset, sortBy }) => {
					...
					if (where && where.length > 0) {
						...
						if (idCondition) {
							...
							if (op === "in") {
								const ids = (Array.isArray(idCondition.value) ? idCondition.value : [idCondition.value]).filter((id): id is string => typeof id === "string");
								if (debugLogs) {
									console.log(`[Firestore Adapter] FINDMANY ${model}:`, { where, ids, note: "using direct doc lookups for IDs" });
								}
							...
							if (op === "notIn" || op === "not_in") {
								if (debugLogs) {
									console.log(`[Firestore Adapter] FINDMANY ${model}:`, { where, note: "handling notIn for IDs" });
								}
								// Get all documents, then filter out the excluded IDs
								const excludedIds = Array.isArray(idCondition.value) ? idCondition.value : [idCondition.value];
					...
					if (hasOrConnector && where) {
						...
						const notInCondition = where?.find((w) => (w.operator as string) === "notIn" || (w.operator as string) === "not_in");
						if (notInCondition) {
							const fieldName = notInCondition.field;
							const arr = Array.isArray(notInCondition.value) ? notInCondition.value : [notInCondition.value];
							results = results.filter((r: any) => !arr.includes(r[fieldName]));
						}
						
						const endsWithCondition = where?.find((w) => (w.operator as string) === "endsWith" || (w.operator as string) === "ends-with" || (w.operator as string) === "ends_with");
						...
						const containsCondition = where?.find((w) => (w.operator as string) === "contains");
					...
					if (hasNotIn || hasEndsWith || hasContains) {
						...
						if (hasContains && containsCondition) {
							...
							const whereWithoutContains = where?.filter((w) => w !== containsCondition);
				...
				count: async ({ model, where }) => {
					...
					if (where && where.length > 0) {
						...
						if (idCondition) {
							...
							// Handle "notIn" operator with IDs
							if (op === "notIn" || op === "not_in") {
								const excludedIds = (Array.isArray(idCondition.value) ? idCondition.value : [idCondition.value]).filter((id): id is string => typeof id === "string");
								const snap = await col.get();
								return snap.docs.filter((doc) => !excludedIds.includes(doc.id)).length;
							}
